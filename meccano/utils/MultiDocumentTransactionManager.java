...
public class MultiDocumentTransactionManager {
    ...
    public MultiDocumentTransactionManager(CBconfig db){
        ...
        if (this.db == null) {
            System.err.println("[ERROR] MultiDocumentTransactionManager: CBconfig is null");
            return;
        }
        ...
        for (int i=0; i< updated_ids.size();i++){
           JsonDocument original = this.getOriginal(updated_ids.get(i));
            if (original!=null)
                ...
                System.err.println("[ERROR] MultiDocumentTransactionManager: Rollback error");
    ...
    public boolean createLockDocument(String document_id){
        ...
        try {
            ...
            log.debug("[TX] Se guarda el documento "+ lock_document.id());
        }
        catch (Exception e){
            System.err.println("[ERROR] MultiDocumentTransactionManager: "+document_id+" already blocked");
            return false;
        }
